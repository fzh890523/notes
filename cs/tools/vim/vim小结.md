...
> - `Ctrl + w + l`：将焦点切换到右侧窗口。
>
> 您也可以在这些命令中添加数字参数，以使焦点在窗口之间移动更多次。例如，`Ctrl + w + 2j` 将焦点向下移动两个窗口。
...
> 在 Vim 中打开文件的命令是 `:edit` 或者简写为 `:e`。您可以通过以下方式使用它：
>
> 1. 打开 Vim 编辑器：在终端中输入 `vim` 命令并按 Enter 键。
> 2. 在 Vim 中输入命令模式：按 `Esc` 键进入命令模式。
> 3. 打开文件：输入 `:e` 命令，后面跟上文件名和路径（如果需要）。
> 4. 按 Enter 键确认：按 Enter 键，Vim 将打开该文件并显示在屏幕上。
>
> 例如，要在 Vim 中打开名为 "example.txt" 的文件，您可以输入以下命令：
>
> ```
> :e example.txt
> ```
>
> 如果文件不在当前目录中，您可以使用文件的完整路径：
>
> ```
> :e /path/to/example.txt
> ```
>
> 您也可以在打开文件时使用 Vim 的其他选项和参数，例如在只读模式下打开文件或垂直分割窗口等。请参考 Vim 的文档以了解更多信息。

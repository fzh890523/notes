...
We've taken [Hundt's benchmark programs](http://code.google.com/p/multi-language-bench/) in C++ and Go, combined each into a single source file, and removed all but one line of output. We'll time the program using Linux's `time` utility with a format that shows user time, system time, real time, and maximum memory usage:
...
To start tuning the Go program, we have to enable profiling. If the code used the [Go testing package](http://golang.org/pkg/testing/)'s benchmarking support, we could use gotest's standard `-cpuprofile` and `-memprofile` flags. In a standalone program like this one, we have to import `runtime/pprof` and add a few lines of code:
...
The new code defines a flag named `cpuprofile`, calls the [Go flag library](http://golang.org/pkg/flag/) to parse the command line flags, and then, if the `cpuprofile` flag has been set on the command line, [starts CPU profiling](http://golang.org/pkg/runtime/pprof/#StartCPUProfile) redirected to that file. The profiler requires a final call to [`StopCPUProfile`](http://golang.org/pkg/runtime/pprof/#StopCPUProfile) to flush any pending writes to the file before the program exits; we use `defer` to make sure this happens as `main` returns.
...
The `go tool pprof` program is a slight variant of [Google's `pprof` C++ profiler](https://code.google.com/p/gperftools/wiki/GooglePerformanceTools). The most important command is `topN`, which shows the top `N` samples in the profile:
...
A small fragment of [the full graph](http://benchgraffiti.googlecode.com/hg/havlak/havlak1.svg) looks like:
...
(See the [diff between `havlak1` and `havlak2`](http://code.google.com/p/benchgraffiti/source/diff?name=34f7624bb2e2&r=240c155236f9&format=unidiff&path=/havlak/havlak.go))
...
(See the [diff from havlak2](http://code.google.com/p/benchgraffiti/source/diff?name=240c155236f9&r=796913012f93&format=unidiff&path=/havlak/havlak.go))
...
(See the [diff from havlak3](http://code.google.com/p/benchgraffiti/source/diff?name=796913012f93&r=d856c2f698c1&format=unidiff&path=/havlak/havlak.go))
...
Of course, it's no longer fair to compare this Go program to the original C++ program, which used inefficient data structures like `set`s where `vector`s would be more appropriate. As a sanity check, we translated the final Go program into [equivalent C++ code](http://code.google.com/p/benchgraffiti/source/browse/havlak/havlak6.cc). Its execution time is similar to the Go program's:
...
(See [havlak6.cc](http://code.google.com/p/benchgraffiti/source/browse/havlak/havlak6.cc) and [havlak6.go](http://code.google.com/p/benchgraffiti/source/browse/havlak/havlak6.go))
...
The program sources, Linux x86-64 binaries, and profiles used to write this post are available in the [benchgraffiti project on Google Code](http://code.google.com/p/benchgraffiti/).

As mentioned above, [`go test`](http://golang.org/cmd/go/#Test_packages) includes these profiling flags already: define a [benchmark function](http://golang.org/pkg/testing/) and you're all set. There is also a standard HTTP interface to profiling data. In an HTTP server, adding
...
来源： <<http://blog.golang.org/profiling-go-programs>>
